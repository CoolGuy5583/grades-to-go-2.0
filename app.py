...
def register():
    ...
    if form.validate_on_submit():
        ...
        user = User(first_name=form.first_name.data, last_name=form.last_name.data, grade=form.grade.data, username=form.username.data, email=form.email.data, password=password, )
        db.session.add(user)
        db.session.commit()
...
@login_required
def account():
    return render_template('account.html', showNav=True, title='GTG-Account')

@app.route('/gpa')
def calculator():
    return render_template('calculator.html', title='GTG - GPA Calculator')
